...
- **Orb** (`HelixOrb.tsx`, 2026-06-16): **Arc-reactor tech core + flowing neuron field** — pure symbolism for Helix, NOT a data view. Core: white-hot inverse-fresnel core sphere, additive gold↔aqua energy halo, two counter-spinning concentric `torus` rings (gold/aqua), a 14-segment aqua coil ring (stylized arc-reactor coil). Surrounding it: **280 neuron `Points`** distributed on a shell, moving **in unison** via a shared flow field + orbital velocity + radial spring (cloud orbits the core as one organism), additive glow shader. **Hover lights up the hovered section** (invisible raycast catcher → per-point `aBright` radial falloff within `HOVER_RADIUS`). Palette strictly gold `#ffc247` / white-hot `#ffe9a8` / aqua `#3fe3d0` (no cyan). Keeps `voiceState`/`audioLevel` reactivity (idle calm · listening brighter aqua shimmer · thinking faster/brighter core · speaking driven by audioLevel — brightness + scale pulse + flow speed). **SVG connection beams removed.** Top tagline **THINK·PLAN·ACT·ELEVATE** + **HELIX·CORE INTELLIGENCE** label. *(Replaced the rejected molten-fire iteration.)*
