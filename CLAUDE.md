...
- **Orb** (`HelixOrb.tsx`, 2026-06-16): **cinematic 3D double helix** — pure symbolism for Helix (the name = a double helix), NOT a data view. Two intertwined `TubeGeometry` strands (white + gold) over ~3.4 turns swept along a `CatmullRomCurve3`, 16 thin DNA "rung" cylinders, slight cinematic tilt + slow Y-spin. **~50 free-flowing dots** (one `InstancedMesh`) drift on their own (orbital + smooth-noise wobble) around the helix, each with a thin faint connector `LineSegments` to a point on the helix (tracks both the drifting dot and the spinning helix). **Two counter-rotating aqua `#3fe3d0` torus rings** (radius 1.15 & 1.3) on crossed tilted planes encircling the helix, spinning opposite directions. Lit (`MeshStandardMaterial` + ambient/key/rim lights) for dimensional shading — **no glow/bloom**. Palette: white + gold `#ffc247`/`#ffe9a8` + aqua `#3fe3d0` rings. Keeps `voiceState`/`audioLevel` reactivity (spin/drift/brightness ease up in thinking/speaking; calm). *(Replaced the rejected neuron-sphere → fire → arc-reactor iterations. Tagline, HELIX label, SVG beams, and the CommandBar window all removed.)*
