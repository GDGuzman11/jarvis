...
- **Orb** (`HelixOrb.tsx`, 2026-06-16): **molten fiery core** — pure symbolism for Helix, NOT a data view. An `IcosahedronGeometry` core with a custom `ShaderMaterial` (inlined simplex/FBM GLSL noise, no dep) roiling in a **champagne-gold fire gradient** (deep gold → `#ffc247` → `#ffe9a8` → white-hot crests) + fresnel-displaced surface, an additive fresnel **corona**, time-noise **flicker** + breathing **pulse**. Keeps `voiceState`/`audioLevel` reactivity (idle calm · listening cooler aqua shimmer · thinking hotter/faster · speaking driven by audioLevel). **SVG connection beams removed** (replaced the old neuron-sphere). Top tagline **THINK·PLAN·ACT·ELEVATE** + **HELIX·CORE INTELLIGENCE** label.
