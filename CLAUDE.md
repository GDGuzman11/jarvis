...
- **Next Task**: **Phase 16F** — Memory Graph Visualization / Window 6 (`/backend-agent` API + `/frontend-agent` graph UI). Build `GET /api/memory/graph`, the `MemoryWindow.tsx` force-directed graph, hover HUD panel, Tauri window registration, and `memory_update` WS event. (16E ColBERT/self-reflection is optional — schedule when ready.)
