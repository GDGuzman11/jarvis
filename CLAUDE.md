...
- **ORB FINALIZED + misc (2026-06-16)** ✅: Central orb iterated to the user-approved **"Ethereal Halo"** (see Design Aesthetic → Orb); **power button split into its own `ShutdownWindow`** (52×52); orb window enlarged 1.4× (504×504); `CommandBarWindow` removed.
- **OPEN-LOOP REMINDER BUG FIXED (2026-06-16)** ✅: Reminders (`open_loops`) were write-only (created, never resolved) so the startup greeting nagged forever, and the keyword `detect_open_loops` stored garbage fragments ("anymore", "s, no need to"). Fix: new `resolve_open_loops_async` (`database.py`); open-loop create/resolve routed through the 16C `LLMExtractor` (`extract_open_loops`) with a pre-filter gate — genuine reminders only, "remove/forget that reminder" resolves the matching loop and never re-creates one; brittle keyword detector demoted to fallback-only (no LLM judge). **Randomized startup greeting** (8 angles via `random.choice`) — varied each launch, still appends overdue reminders (omits resolved). Verified 7/7 by debugger-agent. Suite **210/210** (+14 tests). *(6 pre-existing junk reminders manually dismissed in the live DB at user request.)*
...
- **MEMORY BRAIN POLISH (2026-06-17)** ✅: **Recall-flash** — `MemoryRecallEvent{fact_ids}` emitted from `recall()` (fire-and-forget, no hot-path latency); the Memory window flashes recalled neurons (white-hot pulse, ~1.5s decay, multiple flash together). **Hover fix** — core/glow/edge/tether made non-pickable so only neurons are hover targets (was: core/lines stole the raycast → no panel). **Legacy domain backfill** — startup task re-classifies pre-`domain` facts via the LLM (fixed "mom's name under Project": legacy NULL-domain fact mis-colored by the fallback); fallback map corrected (`correction`/`decision`/`failure`→general, not project). Verified 6/6 by debugger-agent. Suite **243/243**.
- **Test State**: **243/243 passing** (secret-scan green; +10 recall-flash/backfill tests, 2026-06-17) · `pnpm build` clean.
...
- **Window layout**: orb (AnimationWindow) **504×504** at `x:708, y:228` (enlarged 1.4× 2026-06-16); Reasoning left `x:200`; Communications upper-right `x:1180`; Agents bottom `x:200, y:760`; Tools lower-right `x:1180, y:540`; **Power/shutdown button is its own tiny 52×52 window** (`ShutdownWindow.tsx`, label `shutdown`, `x:1216, y:228`, draggable) split out of the orb window. *(CommandBar window removed 2026-06-16 — served no purpose, wasn't movable)*
- **Orb** (`HelixOrb.tsx`, 2026-06-16 — FINAL, user-approved "Ethereal Halo"): pure symbolism for Helix, NOT a data view. **Two crossed broken "fractured" halos** rendered as **white glowing outlines** (`EdgesGeometry` → `LineSegments`, emissive white): Halo A tumbles on its **X-axis**, Halo B spins on its **Y-axis** (slow gyroscope). **Gold center**: emissive gold nucleus (`#ffc247`/`#ffe9a8`) + gold inner orbital-ring outlines + an additive gold glow sprite. **~40 ethereal white dots** (one shader-driven `Points`) drift continuously around it; **distance-based glow** — each dot brightens/enlarges and warms toward gold as it nears the core, fades to faint white far out; **each dot has a faint additive connector `LineSegments` to the core** sharing the same distance falloff (bright at the core end, fading outward). **Glow is alpha-safe additive sprites + emissive only — NO `EffectComposer`/bloom** (bloom broke the transparent window → dark square; removed, `@react-three/postprocessing` uninstalled). Transparency enforced in `AnimationWindow.tsx` Canvas (`gl={{alpha:true, premultipliedAlpha:false}}` + `setClearAlpha(0)` + `scene.background=null`). **Voice reactivity hits the core AND both halos** (synchronized brightness + subtle scale + rotation-speed pulse driven by `audioLevel` while speaking; calm idle). Window enlarged to **504×504** (1.4×). Palette: white outlines + gold center. *(Replaced the rejected neuron-sphere → fire → arc-reactor → double-helix → gold-glass-halo iterations. Tagline, HELIX label, SVG beams, CommandBar window all removed.)*
  - **NOTE — two distinct orbs, do not conflate**: this central orb is *symbolic only*. The **real data-driven memory brain (Phase 16F)** is a SEPARATE future build — neurons = actual memory facts, one neuron added per memory, grows with the DB; force-floating dots tethered to a core, proximity edges, hover → memory contents.
...
> ✅ **COMPLETE 2026-06-17 — but this section's spec is SUPERSEDED.** It shipped as a **data-driven 3D memory brain** (R3F, immersive, gold/white/red/aqua, nodes = `memory`+`person` extensible, bloom in the opaque `memory` window), NOT the flat `react-force-graph-2d` / cyan-purple-green design written below. See the **PHASE 16F COMPLETE** entry in Current Status for what was actually built. The detailed spec below is kept only as historical record — do not implement it.

*(Original spec — historical, superseded.) Handled by: `/backend-agent` (API endpoint) + `/frontend-agent` (graph UI). Depends on 16A + 16B being complete — needs quality columns (access_count, importance, confidence) for node sizing/coloring, and multi-signal scores for edge weights. Visual design: force-directed web, color by type.*
